/*****************************************************************************
 * GENERATED CLASS - ALL CHANGES ARE LOST AFTER RECONFIGURATION OF THE VOLE
 * PROJECT.
 *
 * DO NOT EDIT THIS FILE (UNLESS YOU REALLY KNOW WHAT YOU DO) - IF IT DOES NOT
 * SUIT YOUR NEEDS, CHANGE THE APPROPRIATE VOLE COMPONENT THAT LEFT ITS TRACES
 * HERE!
 ****************************************************************************/

#include "modules.h"

namespace vole {

Modules::Modules() {
	Command *c;

	
	/* RGB true-color or false-color from multispectral images */
	c = new gerbil::RGB(); insert(std::make_pair(c->getName(), c));
	/* Distance to each other pixel in the image in feature space */
	c = new Distance(); insert(std::make_pair(c->getName(), c));
	/* Edge detection in multispectral images */
	c = new EdgeDetection(); insert(std::make_pair(c->getName(), c));
	/* Distance measures for grayscale images */
	c = new vole::CommandSimilarityMeasure(); insert(std::make_pair(c->getName(), c));

}

}
